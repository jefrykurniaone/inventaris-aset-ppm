...
1. **Fix the five defects** â€” refresh the router cache on sign-in/sign-out, move the Vercel functions to the same region as the database and cut redundant/serial queries, draw loading feedback on every navigation, accept a photo during asset creation, and require a reason when deactivating a user.
2. **Bring every list to the same standard** â€” newest-first by default where recency is what the user wants, urgency-first where it is not (loans), clickable column-header sorting on a curated set of columns, a page-size selector on every table, and searchable comboboxes on the dropdowns that grow with real data.
3. **Prove it** â€” a scripted Lighthouse pass with pass/fail budgets recorded in the repository, and a focused penetration review (authorization audit plus an OWASP ZAP baseline scan) once the fixes have merged.
...
- **Deactivation reason**: "deactivate" remains the product term (Better Auth ban underneath â€” see the domain glossary). The deactivate control opens a dialog with a required reason field, stored in the existing ban-reason column that the schema already carries but nothing writes. The reason is shown in the admin users table, admins only. Reactivation clears the stored reason; both actions write an activity log entry carrying the reason so history survives. Reason text is never exposed on any non-admin surface.
...
- **Dropped**: load/stress testing is explicitly not done â€” the production database runs on the Supabase free plan and a load test would exhaust the connection pool for real users.

## Testing Decisions

- Tests assert external behaviour, never implementation: a list's rendered order, a URL's effect on a query, a dialog refusing to submit without a reason â€” not which function was called.
...
- The photo-on-create path extends the existing Playwright smoke test, which already performs a real sign-in, creates an asset, and uploads a photo â€” the smoke path moves the upload into the create step.
...
- Any new deactivation-related schema column â€” the existing ban fields are sufficient.
- Fixing findings from the penetration review inside the review ticket â€” each finding becomes its own issue.
