...
const NEEDS_ACTION_CARD_KEYS = ["overdue", "attention"] as const;
...
function NeedsActionCardSkeleton() {
  return (
...
function NeedsActionRowSkeleton() {
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      {NEEDS_ACTION_CARD_KEYS.map((key) => (
        <NeedsActionCardSkeleton key={key} />
      ))}
    </div>
  );
}
 ...
 * grid shapes — `DashboardSummaryCards`, the two-column needs-action row of
 * `OverdueLoansCard` and `DashboardAttentionCard` (spec #138),
